...
export function getPriceDistribution(smartphones: Smartphone[]): PriceRange[] {
  ...
  const distribution = ranges.map((range) => ({
    ...range,
    count: smartphones.filter((phone) => phone.price >= range.min && phone.price < range.max).length,
  }));
...
export function getBrandSpecsComparison(smartphones: Smartphone[]): BrandSpecs[] {
  const brandMap = new Map<string, { totalRam: number; totalStorage: number; totalBattery: number; count: number }>();
